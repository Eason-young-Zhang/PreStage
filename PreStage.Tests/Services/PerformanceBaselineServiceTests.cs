...
public class PerformanceBaselineServiceTests
{
    private static readonly byte[] MinimalJpeg = new byte[]
    {
        ...
        0xDA, 0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3F, 0x00, 0x7B, 0x94, 0x11, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xD9
    };

    [Fact]
    public void BenchmarkTempFolder_200Files()
    {
        var folder = CreateTempFolderWithJpegs(200);
        try
        {
            var metrics = PerformanceBaselineService.MeasureScanAndRender(folder, false);
            Assert.Equal(200, metrics.FileCount);
            Assert.True(metrics.ScanTimeMs < 10000, $"ScanTimeMs={metrics.ScanTimeMs:F0}ms exceeded 10s limit");
        }
        finally
        {
            SafeDelete(folder);
    ...
    [Fact]
    public void BenchmarkTempFolder_500Files()
    {
        var folder = CreateTempFolderWithJpegs(500);
        try
        {
            var metrics = PerformanceBaselineService.MeasureScanAndRender(folder, false);
            Assert.Equal(500, metrics.FileCount);
            Assert.True(metrics.ScanTimeMs < 10000, $"ScanTimeMs={metrics.ScanTimeMs:F0}ms exceeded 10s limit");
        }
        finally
        {
    ...
    private static string CreateTempFolderWithJpegs(int count)
    {
        var folder = Path.Combine(Path.GetTempPath(), $"prestage_perf_{Guid.NewGuid()}");
        Directory.CreateDirectory(folder);
        for (var i = 0; i < count; i++)
        {
            File.WriteAllBytes(Path.Combine(folder, $"img_{i:D4}.jpg"), MinimalJpeg);
        }
        return folder;
    }
